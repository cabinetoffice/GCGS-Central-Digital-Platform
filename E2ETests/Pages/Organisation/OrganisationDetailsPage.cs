...
public class OrganisationDetailsPage
{
    ...
    public async Task AssertOrganisationName(string name)
    {
        var organisationName = await GetOrganisationName();
        if (organisationName != name)
            throw new Exception($"❌ Expected organisation name '{name}', but got '{organisationName}'");
        Console.WriteLine($"✅ Organisation matches expected value: {name}");
    }
}
