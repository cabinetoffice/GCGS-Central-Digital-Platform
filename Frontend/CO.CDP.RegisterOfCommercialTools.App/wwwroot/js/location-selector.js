const LocationSelector = (() => {
    ...
    const createInstance = (options) => {
        const config = {
            ...
            triggerId: 'browse-location-codes',
        ...
        const saveToSessionStorage = () => {
            ...
            const hasAllNames = codes.every(code => state.locationNames[code]);

            if (hasAllNames && codes.length > 0) {
                sessionStorage.setItem(getStorageKey('selected-codes'), JSON.stringify(codes));
                sessionStorage.setItem(getStorageKey('location-names'), JSON.stringify(state.locationNames));
            } else if (codes.length === 0) {
                sessionStorage.removeItem(getStorageKey('selected-codes'));
                sessionStorage.removeItem(getStorageKey('location-names'));
            }
        };
        ...
        const loadFromSessionStorage = () => {
            try {
                ...
                const storedNames = sessionStorage.getItem(getStorageKey('location-names'));
                if (storedNames) {
                    state.locationNames = JSON.parse(storedNames);
                }
            } catch (e) {
                state.selectedCodes.clear();
                state.locationNames = {};
        ...
        const updateTriggerText = () => {
            ...
            const originalText = state.trigger.dataset.originalText || `Browse locations`;
            ...
            state.trigger.textContent = count > 0
                ? `Edit selected ${config.codeType.toLowerCase()}s`
                : originalText;
        };
        ...
        const updateAccordionContent = () => {
            ...
            const getLocationName = (code) => {
                return state.locationNames[code] || code;
            };
            ...
            selectedCodesDisplay.innerHTML = `
                ...
                <div class="govuk-!-margin-top-2">
                    ${getCurrentSelection().sort((a, b) => {
                        const nameA = getLocationName(a);
                        const nameB = getLocationName(b);
                        return nameA.localeCompare(nameB);
                    }).map(code => {
                        const name = getLocationName(code);
                        return `<div class="govuk-tag govuk-tag--grey govuk-!-margin-right-1 govuk-!-margin-bottom-1">${escapeHtml(name)}</div>`;
                    }).join('')}
                </div>
            `;
        };
        ...
        const updateSelectionDisplay = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    tempDiv.querySelectorAll('.code-selected-tag').forEach(tag => {
                        const code = tag.dataset.code;
                        const nameElement = tag.querySelector('.code-selected-tag__title');
                        if (code && nameElement) {
                            state.locationNames[code] = nameElement.textContent.trim();
                        }
                    });
        ...
        const toggleSelection = async (code) => {
        ...
        const setupTreeEventHandlers = () => {
            if (state.treeContainer && !state.treeContainer.dataset.delegatedListeners) {
                ...
                state.treeContainer.addEventListener('change', async (e) => {
                    if (e.target.matches('[data-action="toggle-selection"]') && !e.target.disabled) {
                        e.stopPropagation();
                        await toggleSelection(e.target.dataset.code);
                    }
                });
            ...
            document.querySelectorAll(`#${config.searchContainerId} [data-action="toggle-selection"]`).forEach(item => {
                const handleToggle = async () => {
                    if (!item.hasAttribute('aria-disabled')) {
                        await toggleSelection(item.dataset.code);
                        const searchResults = document.getElementById(`${config.searchInputId}-results`);
                        if (searchResults) {
                            searchResults.classList.add('govuk-!-display-none');
                            searchResults.classList.remove('govuk-!-display-block');
        ...
        const setupSelectionEventHandlers = () => {
            document.querySelectorAll('[data-action="remove-selection"]').forEach(button => {
                button.addEventListener('click', async () => {
                    await toggleSelection(button.dataset.code);
                });
            });
        };
        ...
        loadSelectedCodes();
        updateTriggerText();
        if (state.selectedCodes.size > 0 && Object.keys(state.locationNames).length > 0) {
            updateAccordionContent();
        }
        setupModalHandlers();
