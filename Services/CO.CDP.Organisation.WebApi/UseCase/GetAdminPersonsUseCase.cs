...
public class GetAdminPersonsUseCase(IPersonRepository personRepository, IOrganisationRepository organisationRepository, IMapper mapper)
    : IUseCase<Guid, IEnumerable<Model.Person>>
{
    public async Task<IEnumerable<Model.Person>> Execute(Guid organisationId)
    {
        ...
        foreach (var person in persons)
        {
            ...
            if (personModel.Scopes.Select(item => item.ToLower()).Contains("ADMIN".ToLower()))
            {
