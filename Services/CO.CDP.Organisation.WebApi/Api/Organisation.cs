...
public static class EndpointExtensions
{
    ...
    public static void UseOrganisationEndpoints(this WebApplication app)
    {
        app.MapGet("/organisations",
                    ...
                    IUseCase<PaginatedOrganisationQuery, Tuple<IEnumerable<OrganisationDto>, int>> useCase) =>
                {
                    return await useCase
                        .Execute(new PaginatedOrganisationQuery(limit, skip, role, pendingRole, searchText))
                ...
                async (RegisterOrganisation command, IUseCase<RegisterOrganisation, Model.Organisation> useCase) =>
                    await useCase.Execute(command)
                        .AndThen(organisation =>
                            organisation != null
                                ? Results.Created(new Uri($"/organisations/{organisation.Id}", UriKind.Relative),
                                    organisation)
        ...
        app.MapPost("/organisations/{organisationId}/join-requests",
                [OrganisationAuthorize([AuthenticationChannel.OneLogin])]
                async (Guid organisationId, CreateOrganisationJoinRequest command,
                        IUseCase<(Guid, CreateOrganisationJoinRequest), OrganisationJoinRequest> useCase) =>
        ...
        app.MapGet("/organisations/{organisationId}/join-requests",
                ...
                async (Guid organisationId, [FromQuery] OrganisationJoinRequestStatus? status,
                        IUseCase<(Guid, OrganisationJoinRequestStatus?), IEnumerable<JoinRequestLookUp>> useCase) =>
                    await useCase.Execute((organisationId, status))
                        .AndThen(organisations =>
                            organisations != null ? Results.Ok(organisations) : Results.NotFound()))
        ...
        app.MapPatch("/organisations/{organisationId}/join-requests/{joinRequestId}",
                ...
                async (Guid organisationId, Guid joinRequestId, UpdateJoinRequest updateJoinRequest,
                        IUseCase<(Guid, Guid, UpdateJoinRequest), bool> useCase) =>
    ...
    public static RouteGroupBuilder UseOrganisationLookupEndpoints(this RouteGroupBuilder app)
    {
        ...
        app.MapGet("/lookup",
                ...
                async ([FromQuery] string? name, [FromQuery] string? identifier,
                        IUseCase<OrganisationQuery, Model.Organisation?> useCase) =>
        ...
        app.MapGet("/search",
                [OrganisationAuthorize([AuthenticationChannel.OneLogin, AuthenticationChannel.ServiceKey])]
                async ([FromQuery] string name, [FromQuery] string? role, [FromQuery] int limit,
                    [FromServices] IUseCase<OrganisationSearchQuery, IEnumerable<OrganisationSearchResult>> useCase,
                    [FromQuery] double? threshold = 0.3, [FromQuery] bool includePendingRoles = false) =>
                {
                    ...
                    return await useCase
                        .Execute(new OrganisationSearchQuery(name, limit, threshold, role, includePendingRoles))
                        .AndThen(results => results.Count() != 0 ? Results.Ok(results) : Results.NotFound());
                })
            .Produces<IEnumerable<OrganisationSearchResult>>(StatusCodes.Status200OK, "application/json")
            .Produces<ProblemDetails>(StatusCodes.Status401Unauthorized)
            .Produces<ProblemDetails>(StatusCodes.Status404NotFound)
            .Produces<ProblemDetails>(StatusCodes.Status500InternalServerError)
            .Produces<ProblemDetails>(StatusCodes.Status400BadRequest)
                ...
                foreach (var parameter in operation.Parameters)
                {
                    if (parameter.Name == "threshold")
                    {
                        parameter.Description =
                            "The word similarity threshold value for fuzzy searching - Value can be from 0 to 1";
                    }
        ...
        app.MapGet("/find/by-organisation-email",
                [OrganisationAuthorize([AuthenticationChannel.OneLogin, AuthenticationChannel.ServiceKey])]
                async ([FromQuery] string email, [FromQuery] string? role, [FromQuery] int limit,
                        [FromServices]
                        IUseCase<OrganisationsByOrganisationEmailQuery, IEnumerable<OrganisationSearchResult>>
                            useCase) =>
        ...
        app.MapGet("/find/by-admin-email",
                ...
                async ([FromQuery] string email, [FromQuery] string? role, [FromQuery] int limit,
                        [FromServices]
                        IUseCase<OrganisationsByAdminEmailQuery, IEnumerable<OrganisationSearchResult>> useCase) =>
    ...
    public static RouteGroupBuilder UseSupportEndpoints(this RouteGroupBuilder app)
    {
        app.MapPatch("/organisation/{organisationId}",
                [OrganisationAuthorize([AuthenticationChannel.OneLogin],
                    personScopes: [Constants.PersonScope.SupportAdmin])]
                async (Guid organisationId, SupportUpdateOrganisation supportUpdateOrganisation,
                        IUseCase<(Guid, SupportUpdateOrganisation), bool> useCase) =>
            ...
            .WithOpenApi(operation =>
            {
                ...
                operation.Description =
                    "Update an organisation as a support admin. Perform support admin specific actions.";
    ...
    public static RouteGroupBuilder UseFeedbackEndpoints(this RouteGroupBuilder app)
    {
        app.MapPost("/feedback/",
            ...
            .WithOpenApi(operation =>
            {
                ...
                operation.Summary =
                    "Ask a question, report a problem or suggest an improvement to the Find a Tender service team.";
    ...
    public static RouteGroupBuilder UsePersonsEndpoints(this RouteGroupBuilder app)
    {
        app.MapGet("/{organisationId}/persons",
            ...
            .Produces<List<WebApiPerson>>(StatusCodes.Status200OK, "application/json")
            .Produces<ProblemDetails>(StatusCodes.Status401Unauthorized)
            .Produces<ProblemDetails>(StatusCodes.Status404NotFound)
            .ProducesProblem(StatusCodes.Status422UnprocessableEntity)
            .Produces<ProblemDetails>(StatusCodes.Status500InternalServerError)
        ...
        app.MapPatch("/{organisationId}/persons/{personId}",
                [OrganisationAuthorize(
                    ...
                    [Constants.OrganisationPersonScope.Admin],
                    OrganisationIdLocation.Path)]
                async (Guid organisationId, Guid personId, UpdatePersonToOrganisation updatePersonToOrganisation,
                        IUseCase<(Guid, Guid, UpdatePersonToOrganisation), bool> useCase) =>
        ...
        app.MapDelete("/{organisationId}/persons",
                [OrganisationAuthorize(
                    ...
                    [Constants.OrganisationPersonScope.Admin],
                    OrganisationIdLocation.Path)]
                async (Guid organisationId, [FromBody] RemovePersonFromOrganisation removePersonFromOrganisation,
                        IUseCase<(Guid, RemovePersonFromOrganisation), bool> useCase) =>
        ...
        app.MapGet("/{organisationId}/invites",
                ...
                async (Guid organisationId, IUseCase<Guid, IEnumerable<PersonInviteModel>> useCase) =>
                    await useCase.Execute(organisationId)
                        .AndThen(personInvites =>
                            personInvites != null ? Results.Ok(personInvites) : Results.NotFound()))
        ...
        app.MapPost("/{organisationId}/invites",
                ...
                async (Guid organisationId, InvitePersonToOrganisation invitePersonToOrganisation,
                        IUseCase<(Guid, InvitePersonToOrganisation), bool> useCase) =>
        ...
        app.MapPatch("/{organisationId}/invites/{personInviteId}",
                [OrganisationAuthorize(
                    ...
                    OrganisationIdLocation.Path,
                    apiKeyScopes: [Constants.ApiKeyScopes.WriteOrganisationData])]
                async (Guid organisationId, Guid personInviteId,
                        UpdateInvitedPersonToOrganisation updateInvitedPersonToOrganisation,
                        IUseCase<(Guid, Guid, UpdateInvitedPersonToOrganisation), bool> useCase) =>
        ...
        app.MapPost("/{organisationId}/services/invites",
                [OrganisationAuthorize(
                    ...
                    apiKeyScopes: [Constants.ApiKeyScopes.WriteOrganisationData])]
                async (Guid organisationId, InvitePersonToOrganisation invitePersonToOrganisation,
                        IUseCase<(Guid, InvitePersonToOrganisation), PersonInvite> useCase) =>
                    await useCase.Execute((organisationId, invitePersonToOrganisation))
            ...
            .WithOpenApi(operation =>
            {
                ...
                operation.Description =
                    "Create a new person invite (service-to-service). Returns the created invite with GUID.";
                operation.Summary = "Create a new person invite for service integration.";
                operation.Responses["200"].Description =
                    "Person invite created successfully. Returns the invite details including GUID.";
                operation.Responses["400"].Description = "Bad request.";
                operation.Responses["401"].Description = "Valid authentication credentials are missing in the request.";
                operation.Responses["404"].Description = "Organisation not found.";
                operation.Responses["422"].Description = "Unprocessable entity.";
        ...
        app.MapPost("/{organisationId}/services/invites/{personInviteId}/resend",
                [OrganisationAuthorize(
                    [AuthenticationChannel.ServiceKey],
                    organisationIdLocation: OrganisationIdLocation.Path,
                    apiKeyScopes: [Constants.ApiKeyScopes.WriteOrganisationData])]
                async (Guid organisationId, Guid personInviteId, ResendPersonInviteUseCase useCase) =>
                    await useCase.Execute((organisationId, personInviteId))
                        .AndThen(_ => Results.NoContent()))
            .Produces(StatusCodes.Status204NoContent)
            .ProducesProblem(StatusCodes.Status400BadRequest)
            ...
            .WithOpenApi(operation =>
            {
                operation.OperationId = "ResendPersonInviteForService";
                operation.Description =
                    "Resend an existing person invite (service-to-service). Extends the invite lifespan and re-sends the notification email.";
                operation.Summary = "Resend a person invite for service integration.";
                operation.Responses["204"].Description = "Invite resent successfully.";
                operation.Responses["400"].Description = "Bad request.";
                operation.Responses["401"].Description = "Valid authentication credentials are missing in the request.";
                operation.Responses["404"].Description = "Organisation or invite not found.";
                operation.Responses["422"].Description = "Unprocessable entity.";
                operation.Responses["500"].Description = "Internal server error.";
                return operation;
            });
    ...
    public static RouteGroupBuilder UseManageApiKeyEndpoints(this RouteGroupBuilder app)
    {
        app.MapGet("/{organisationId}/api-keys",
                [OrganisationAuthorize(
                    ...
                    OrganisationIdLocation.Path,
                    [Constants.PersonScope.SupportAdmin])]
                async (Guid organisationId, IUseCase<Guid, IEnumerable<AuthenticationKey>> useCase) =>
                    await useCase.Execute(organisationId)
    ...
    public static RouteGroupBuilder UseOrganisationMouEndpoints(this RouteGroupBuilder app)
    {
        app.MapGet("/{organisationId}/mou",
                ...
                async (Guid organisationId, IUseCase<Guid, IEnumerable<MouSignature>> useCase) =>
                    await useCase.Execute(organisationId)
                        .AndThen(mouSignatures =>
                            mouSignatures != null ? Results.Ok(mouSignatures) : Results.NotFound()))
            .Produces<List<MouSignature>>(StatusCodes.Status200OK, "application/json")
            .Produces<ProblemDetails>(StatusCodes.Status401Unauthorized)
            .Produces<ProblemDetails>(StatusCodes.Status404NotFound)
            .ProducesProblem(StatusCodes.Status422UnprocessableEntity)
            .Produces<ProblemDetails>(StatusCodes.Status500InternalServerError)
        ...
        app.MapGet("/{organisationId}/mou/{mouSignatureId}",
            ...
            .Produces<MouSignature>(StatusCodes.Status200OK, "application/json")
            .Produces<ProblemDetails>(StatusCodes.Status401Unauthorized)
            .Produces<ProblemDetails>(StatusCodes.Status404NotFound)
            .ProducesProblem(StatusCodes.Status422UnprocessableEntity)
            .Produces<ProblemDetails>(StatusCodes.Status500InternalServerError)
        ...
        app.MapGet("/{organisationId}/mou/latest",
                [OrganisationAuthorize(
                    [AuthenticationChannel.OneLogin],
                    [
                        Constants.OrganisationPersonScope.Admin, Constants.OrganisationPersonScope.Responder,
                        Constants.OrganisationPersonScope.Editor, Constants.OrganisationPersonScope.Viewer
                    ],
                    OrganisationIdLocation.Path,
                    [Constants.PersonScope.SupportAdmin])]
                async (Guid organisationId, IUseCase<Guid, MouSignatureLatest> useCase) =>
                    await useCase.Execute(organisationId)
                        .AndThen(mouSignatureLatest =>
                            mouSignatureLatest != null ? Results.Ok(mouSignatureLatest) : Results.NotFound()))
            .Produces<MouSignatureLatest>(StatusCodes.Status200OK, "application/json")
            .Produces<ProblemDetails>(StatusCodes.Status401Unauthorized)
            .Produces<ProblemDetails>(StatusCodes.Status404NotFound)
            .ProducesProblem(StatusCodes.Status422UnprocessableEntity)
            .Produces<ProblemDetails>(StatusCodes.Status500InternalServerError)
    ...
    public static RouteGroupBuilder UseOrganisationPartiesEndpoints(this RouteGroupBuilder app)
    {
        app.MapGet("/{organisationId}/parties",
                [OrganisationAuthorize(
                    ...
                    [
                        Constants.OrganisationPersonScope.Admin, Constants.OrganisationPersonScope.Editor,
                        Constants.OrganisationPersonScope.Responder, Constants.OrganisationPersonScope.Viewer
                    ],
                    OrganisationIdLocation.Path,
                    [Constants.PersonScope.SupportAdmin])]
        ...
        app.MapPost("/{organisationId}/add-party",
                [OrganisationAuthorize(
                    ...
                    [Constants.OrganisationPersonScope.Admin, Constants.OrganisationPersonScope.Editor],
                    OrganisationIdLocation.Path)]
                async (Guid organisationId, AddOrganisationParty organisationParty,
                        IUseCase<(Guid, AddOrganisationParty), bool> useCase) =>
        ...
        app.MapPost("/{organisationId}/update-party",
                [OrganisationAuthorize(
                    ...
                    [Constants.OrganisationPersonScope.Admin, Constants.OrganisationPersonScope.Editor],
                    OrganisationIdLocation.Path)]
                async (Guid organisationId, UpdateOrganisationParty organisationParty,
                        IUseCase<(Guid, UpdateOrganisationParty), bool> useCase) =>
                    await useCase.Execute((organisationId, organisationParty))
                        .AndThen(_ => Results.NoContent())
            )
            .Produces(StatusCodes.Status204NoContent)
        ...
        app.MapDelete("/{organisationId}/remove-party",
                [OrganisationAuthorize(
                    ...
                    [Constants.OrganisationPersonScope.Admin, Constants.OrganisationPersonScope.Editor],
                    OrganisationIdLocation.Path)]
                async (Guid organisationId, [FromBody] RemoveOrganisationParty organisationParty,
                        IUseCase<(Guid, RemoveOrganisationParty), bool> useCase) =>
                    await useCase.Execute((organisationId, organisationParty))
                        .AndThen(_ => Results.NoContent())
            )
            .Produces(StatusCodes.Status204NoContent)
    ...
    public static RouteGroupBuilder useSearchRegistryOfPpon(this RouteGroupBuilder app)
    {
        app.MapGet("/search-by-name-or-ppon",
                [OrganisationAuthorize([AuthenticationChannel.OneLogin, AuthenticationChannel.ServiceKey])]
                async ([FromQuery] string searchText, [FromQuery] int limit, [FromQuery] int skip,
                    [FromQuery] string sortOrder,
                    [FromServices]
                    IUseCase<OrganisationSearchByPponQuery, (IEnumerable<OrganisationSearchByPponResult>, int)> useCase,
                    [FromQuery] double? threshold = 0.3, [FromQuery] OrganisationSearchFilter? filters = null) =>
                {
                    ...
                    return await useCase
                        .Execute(new OrganisationSearchByPponQuery(searchText, limit, skip, sortOrder, threshold,
                            filters))
                        .AndThen(result =>
                        {
...
public static class ApiExtensions
{
    public static void DocumentOrganisationApi(this SwaggerGenOptions options, IConfigurationManager configuration)
    {
        options.SwaggerDoc("v1", new OpenApiInfo
        {
            ...
            Description =
                "API for creating, updating, deleting, and listing organisations, including a lookup feature against organisation identifiers."
        });
