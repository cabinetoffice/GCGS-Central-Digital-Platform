...
public class GetOrganisationsUseCaseTests
{
    ...
    public async Task Execute_WithValidCommand_ReturnsMappedOrganisations()
    {
        ...
        _organisationRepositoryMock.Setup(repo => repo.GetPaginated(command.Role, command.PendingRole, command.SearchText, command.Limit, command.Skip)).ReturnsAsync(organisations);
