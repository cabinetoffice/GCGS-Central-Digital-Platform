...
{
    /// <inheritdoc />
    public partial class RebackfillFindATenderAccess : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Idempotently fix Find a Tender application role configuration.
            // Ensures sync_to_organisation_information, organisation_information_scopes,
            // and required_party_roles are correctly set regardless of prior migration state.
            migrationBuilder.Sql(
                """
                UPDATE user_management.application_roles ar
                SET
                    sync_to_organisation_information = TRUE,
                    organisation_information_scopes  =
                        CASE ar.name
                    ...
                    required_party_roles =
                        CASE ar.name
                            WHEN 'Editor (buyer)'    THEN ARRAY[1]::integer[]
                            WHEN 'Editor (supplier)' THEN ARRAY[4]::integer[]
                            WHEN 'Viewer (buyer)'    THEN ARRAY[1]::integer[]
                            WHEN 'Viewer (supplier)' THEN ARRAY[4]::integer[]
                        END
                FROM user_management.applications app
                WHERE ar.application_id = app.id
                  AND app.client_id = 'find-a-tender'
                  AND ar.name IN ('Editor (buyer)', 'Editor (supplier)', 'Viewer (buyer)', 'Viewer (supplier)');
                """);

            // Re-backfill Find a Tender access for orgs/users created or modified
            // after the previous backfill migration (20260324060925) ran.
            // Guarded by table existence checks — safe in environments where OI tables are absent.
            migrationBuilder.Sql(
                """
                DO $$
                BEGIN
                    ...
                    AND EXISTS (
                        ...
                        -- Step 1: Ensure Find a Tender organisation_application rows exist for all
                        -- qualifying orgs (those with buyer or tenderer party roles).
                        WITH source_memberships AS (
                            ...
                            WHERE oi_org.roles && ARRAY[1, 4]::integer[]
                        )
                        INSERT INTO user_management.organisation_applications
                            (organisation_id, application_id, is_active, enabled_at, enabled_by,
                        ...
                        SELECT DISTINCT
                            ...
                            TRUE,
                            NOW(),
                            'migration:rebackfill-find-a-tender-access',
                            FALSE,
                            NOW(),
                            'migration:rebackfill-find-a-tender-access'
                        ...
                        SET
                            is_active    = TRUE,
                            enabled_at   = NOW(),
                            enabled_by   = 'migration:rebackfill-find-a-tender-access',
                            disabled_at  = NULL,
                            disabled_by  = NULL,
                            is_deleted   = FALSE,
                            deleted_at   = NULL,
                            deleted_by   = NULL,
                            modified_at  = NOW(),
                            modified_by  = 'migration:rebackfill-find-a-tender-access';

                        -- Step 2: Upsert user_application_assignments for qualifying members.
                        WITH source_memberships AS (
                            SELECT DISTINCT
                                membership.id           AS membership_id,
                                membership.organisation_id,
                                CASE WHEN oi_org.roles @> ARRAY[1]::integer[]      THEN TRUE ELSE FALSE END AS has_buyer_role,
                                CASE WHEN oi_org.roles && ARRAY[4]::integer[]   THEN TRUE ELSE FALSE END AS has_tenderer_role,
                                CASE WHEN COALESCE(op.scopes, '[]'::jsonb) @> '["ADMIN"]'::jsonb THEN TRUE ELSE FALSE END AS has_admin_scope
                            FROM public.organisation_person op
                            INNER JOIN public.persons p
                                ON p.id = op.person_id
                            INNER JOIN public.organisations oi_org
                        ...
                        target_roles AS (
                            SELECT
                                ...
                                CASE
                                    ...
                                    WHEN sm.has_tenderer_role AND sm.has_admin_scope THEN 'Editor (supplier)'
                                    WHEN sm.has_buyer_role              THEN 'Viewer (buyer)'
                                    WHEN sm.has_tenderer_role THEN 'Viewer (supplier)'
                                    ELSE NULL
                        ...
                        INSERT INTO user_management.user_application_assignments
                            (user_organisation_membership_id, organisation_application_id, is_active,
                             assigned_at, assigned_by, is_deleted, created_at, created_by)
                        SELECT DISTINCT
                            rt.membership_id,
                        ...
                        SET
                            is_active   = TRUE,
                            assigned_at = NOW(),
                            assigned_by = 'migration:rebackfill-find-a-tender-access',
                            revoked_at  = NULL,
                            revoked_by  = NULL,
                            is_deleted  = FALSE,
                            deleted_at  = NULL,
                        ...
                        -- Step 3: Remove stale user_assignment_roles for Find a Tender assignments
                        --         that no longer match the target role for the member.
                        WITH resolved_targets AS (
                            ...
                            INNER JOIN user_management.application_roles ar
                               ...
                               AND ar.name = CASE
                                   ...
                                   WHEN oi_org.roles && ARRAY[4]::integer[] AND COALESCE(op.scopes, '[]'::jsonb) @> '["ADMIN"]'::jsonb THEN 'Editor (supplier)'
                                   WHEN oi_org.roles @> ARRAY[1]::integer[]    THEN 'Viewer (buyer)'
                                   WHEN oi_org.roles && ARRAY[4]::integer[] THEN 'Viewer (supplier)'
                                   ELSE NULL
                               END
                            WHERE oi_org.roles && ARRAY[1, 4]::integer[]
                        )
                        ...
                        WHERE uar.user_assignment_id = rt.user_assignment_id
                          ...
                          AND ar.application_id = (
                              SELECT id FROM user_management.applications WHERE client_id = 'find-a-tender'
                          )
                          AND uar.role_id <> rt.target_role_id;

                        -- Step 4: Insert correct user_assignment_roles for Find a Tender assignments.
                        WITH resolved_targets AS (
                            SELECT
                                uaa.id  AS user_assignment_id,
                                ar.id   AS target_role_id
                        ...
                        INSERT INTO user_management.user_assignment_roles (user_assignment_id, role_id)
                        SELECT DISTINCT rt.user_assignment_id, rt.target_role_id
                        FROM resolved_targets rt
