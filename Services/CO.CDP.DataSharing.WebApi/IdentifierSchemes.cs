...
public class IdentifierSchemes : IIdentifierSchemes
{
    private IDictionary<string, string> _schemesToEndpointUris = new Dictionary<string, string>()
    {
    ...
    public IdentifierSchemes(IConfigurationService configurationService)
    {
        SchemesToEndpointUris.Add("GB-PPON", configurationService?.GetOrganisationsApiUrl() + "/{0}" ?? string.Empty);
    }

    public IDictionary<string, string> SchemesToEndpointUris
    {
        get { return _schemesToEndpointUris; }
        private set { _schemesToEndpointUris = value; }
    }

    public Uri? GetRegistryUri(string? scheme, string? identifierId)
    {
