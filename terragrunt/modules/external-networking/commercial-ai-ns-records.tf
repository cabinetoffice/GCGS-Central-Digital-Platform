resource "aws_route53_record" "commercial_ai_cert" {
  count = local.commercial_ai_cert_validator_records[var.environment] != null ? 1 : 0

  name    = local.commercial_ai_cert_validator_records[var.environment].name
  records = local.commercial_ai_cert_validator_records[var.environment].records
  ttl     = 60
  type    = "CNAME"
  zone_id = var.core_hosted_zone_id
}

resource "aws_route53_record" "commercial_ai_a" {
  zone_id = var.core_hosted_zone_id
  name    = "commercial-ai"
  type    = "A"

  alias {
    name                   = local.commercial_ai_cloudfront_domain
    zone_id                = local.cloudfront_global_hosted_zone_id
    evaluate_target_health = false
  }
}

resource "aws_route53_record" "commercial_ai_aaaa" {
  zone_id = var.core_hosted_zone_id
  name    = "commercial-ai"
  type    = "AAAA"

  alias {
    name                   = local.commercial_ai_cloudfront_domain
    zone_id                = local.cloudfront_global_hosted_zone_id
    evaluate_target_health = false
  }
}
