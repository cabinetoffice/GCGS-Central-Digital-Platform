locals {
  ...
  envs_combined_versions = jsonencode({
    ...
    "S:${coalesce(var.pinned_service_versions_sirsi[env], data.aws_ssm_parameter.service_version_sirsi.value)} | F:${coalesce(var.pinned_service_versions_fts[env], data.aws_ssm_parameter.service_version_fts.value) } | F:${coalesce(var.pinned_service_versions_cfs[env], data.aws_ssm_parameter.service_version_cfs.value) }"
  })
