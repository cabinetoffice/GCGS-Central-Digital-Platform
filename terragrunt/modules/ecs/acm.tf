resource "aws_acm_certificate" "this" {
  domain_name               = var.public_beta_domain
  subject_alternative_names = ["*.${var.public_beta_domain}"]
  validation_method         = "DNS"

  tags = merge(var.tags, { Name = var.public_beta_domain })
...
resource "aws_route53_record" "cert_validation" {
  ...
  zone_id         = var.public_beta_hosted_zone_id
}
...
resource "aws_acm_certificate_validation" "public" {
  ...
  validation_record_fqdns = [for record in aws_acm_certificate.public.domain_validation_options : record.resource_record_name]
}
