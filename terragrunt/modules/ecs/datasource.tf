...
data "aws_iam_policy_document" "ecs_task_access_queue" {
  statement {
    ...
    resources = [
      ...
      var.queue_fts_notice_publish_dotnet_arn,
...
data "aws_iam_policy_document" "step_function_manage_services" {
  statement {
    ...
    resources = [
      "arn:aws:ecs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:service/${local.fts_cluster_name}/*",
