resource "aws_route53_record" "services_to_alb" {
  ...
  records = each.value.cluster == "sirsi-php" ? [aws_lb.ecs_php.dns_name] : each.value.cluster == "fts" ? [aws_lb.ecs_fts.dns_name] : [aws_lb.ecs.dns_name]
}

resource "aws_route53_record" "internal_services_to_alb" {
  for_each = local.service_configs
  ...
  alias {
    ...
    name                   = aws_lb.ecs_internal.dns_name
    zone_id                = aws_lb.ecs_internal.zone_id
  }
}
