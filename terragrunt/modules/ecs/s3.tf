...
module "s3_bucket_document_exports" {
  source             = "../s3-bucket"
  bucket_name        = "${local.name_prefix}-${var.environment}-document-exports-${data.aws_caller_identity.current.account_id}"
  enable_encryption  = false
  kms_key_admin_role = var.role_terraform_arn

  # Internal services in this account can write/read (fine-grained access can be layered on later).
  read_roles  = [var.role_ecs_task_exec_arn]
  write_roles = [var.role_ecs_task_arn]
