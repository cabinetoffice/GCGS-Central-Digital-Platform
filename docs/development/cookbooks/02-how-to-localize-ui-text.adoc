...
The next time Visual Studio's resource explorer touches the fil, it may make whitespace changes, particularly around the area of the comment in the XML. These can be hidden when viewing the diff in GitHub if necessary.