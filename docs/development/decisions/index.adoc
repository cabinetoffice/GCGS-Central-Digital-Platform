= Design Decisions

This is a log of important design decisions that were made during development.

The design decision log accomplishes the following goals:

* Provide a structured approach to decision-making.
* Explain to current and future developers why things are the way that they are.
* Help to revise decisions when the situation changes.

:leveloffset: +1

include::0001-record-architecture-decisions.adoc[]

include::0002-create-webapi-clients.adoc[]

include::0003-applying-database-migrations.adoc[]

include::0004-error-handing-and-validation.adoc[]

include::0005-db-naming-conventions.adoc[]

include::0006-acceptance-tests.adoc[]

include::0007-shared-sessions.adoc[]

include::0008-git-merge-strategy.adoc[]

include::0009-outbox-processor-topology-and-fanout.adoc[]

include::0010-typed-outbox-routing.adoc[]

:leveloffset: -1
